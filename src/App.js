...
function computePasses(sat, cty, numDays) {
  const T = orbPeriod(sat.altitude);
...
function CountryTrackMap({ country, sat, width = 500, height = 440 }) {
  ...
  const { passPolys, covPct, passes, passMidTimes } = useMemo(
    () => computePasses(sat, country, tfDays),
    [sat.altitude, sat.inclination, sat.eccentricity, sat.raan, sat.argPerigee,
     sat.swathAngle, sat.offNadir, country.id, tfDays]
  );
      ...
      sats.forEach(sat => {
        ...
        selCtys.forEach(cId => {
          ...
          for (const bucket of passBuckets) {
            passCountByTf[bucket] = allPasses.filter(p => p.day <= bucket).length;
          }
          ...
          for (const bucket of passBuckets) {
            sunlitPassCountByTf[bucket] = sunlitPasses.filter(p => p.day <= bucket).length;
          }
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 8 }}>
                {sats.map(sat => {
                  ...
                  return selCtyObjs.map(cty => (
                    <CountryTrackMap key={`${sat.id}-${cty.id}`} country={cty} sat={effSat} width={500} height={440} />
                  ));
                })}
              </div>
            </P>
